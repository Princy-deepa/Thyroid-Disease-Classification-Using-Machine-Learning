...
pid = request.form['PI']
inp_features=
[[age,np.log(float(sex)),int(thyroxine),int(query),int(antithyro),int(sick
),
int(preg),int(thyros),int(treat), int(query1),int(query2),int(Li),
int(Go), int(Tu),int(hypo),int(Psycho),np.log(float(tsh)),
np.log(float(t3)),
...
prediction_results=prediction_text)
